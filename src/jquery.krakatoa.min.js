(function(e,c,a,h){var g={width:"400px",height:"300px",display:"block",arrows:true,buttons:true,items:1,first:0,gutter:10,loop:false,autoplay:true,direction:1,delay:2500,duration:500,easing:"swing",callback:h};
...
}b.prototype={init:function(){var w=this,k=w.slider,o=w.settings,m=k.children().length,p,v,n,l,u,j,t,q,s,r=0;k.html('<div class="krakatoa-control"></div><div class="krakatoa-container">'+k.html()+"</div>");
...
l=k.width();n=(l-(o.items-1)*o.gutter)/o.items;for(p=0;p<m;p++){v=j.children().eq(p);v.css("width",n);if(o.height==="max"){u=v.outerHeight(true);if(u>r){r=u;
}}}for(p=0;p<o.items&&o.first+p<m;p++){v=j.children().eq(o.first+p);v.addClass("current").css({display:"block",left:(n+o.gutter)*p});if(o.height!=="max"){r=o.height==="auto"?v.outerHeight(true):o.height;
...
k.find(".arrow").on("click touchstart",{krakatoa:w},d);}if(o.buttons){q='<ul class="buttons">';for(p=0;p<m/o.items;p++){q+='<li class="pagination"><a href="#">'+p+"</a></li>";
}q+="</ul>";k.find(".krakatoa-control").append(q);k.find(".buttons a").on("click touchstart",{krakatoa:w},d);k.find(".buttons").find("li").eq(o.first).addClass("active-button");
}}s=c[o.callback];if(typeof s==="function"){s();}if(o.autoplay){w.play();k.on("mouseleave",function(){w.play();});k.on("mouseover",function(){clearTimeout(w.frameId);
w.playing=false;});}},play:function(){var i=this;i.playing=true;i.frameId=c.setTimeout(function(){console.log(i.frameId);if(i.slider.find(":animated").length===0){i.do_the_move(i.settings.direction);
}i.deferred.done(function(){console.log(i.frameId);if(i.playing){i.play();}});},i.settings.delay);},do_the_move:function(v){var y=this,l=this.slider,p=this.settings,j=l.find(".krakatoa-container"),m=j.children().length,s=parseInt(l.data("current")),r=s+p.items*v,o=v/Math.abs(v),q,x,n,k,w,u=0,t=0;
