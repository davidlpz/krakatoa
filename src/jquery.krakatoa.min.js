...
}};c.fn.krakatoa.defaults={width:"400px",height:"300px",display:"block",arrows:true,buttons:true,items:1,first:0,gutter:10,loop:false,autoplay:true,direction:"right",delay:2000,duration:500,easing:"swing"};
...
}else{if(p<0||p>=k){if(n.playing){n.playing=false;}return;}}}else{if(n.buttons){if(w.parent().hasClass("active-button")){return;}m=(w.parent().index()*n.items>r)?1:-1;
...
for(o=0;o<n.items;o++){v=f.children().eq(r+o);v.removeClass("current").animate({left:-(g+n.gutter)*m+(l+n.gutter)*o},n.duration,n.easing,function(){c(this).css({left:0,display:"none"});
});if(p+o>k-1){continue;}j++;v=f.children().eq(p+o);v.addClass("current").css({display:"block",left:(g+n.gutter)*m+(l+n.gutter)*o}).animate({left:(l+n.gutter)*o},n.duration,n.easing,function(){j--;
