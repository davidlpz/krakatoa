...
}g=h.width();k=(g-(l.items-1)*l.gutter)/l.items;if(l.height==="auto-fixed"){for(n=0;n<j;n++){r=f.children().eq(n).height();if(r>p){p=r;}}}for(n=0;n<l.items&&l.first+n<j;
n++){s=f.children().eq(l.first+n);s.addClass("current").css({display:"block",width:k,left:(k+l.gutter)*n});if(l.height!=="auto-fixed"){r=l.height==="auto"?s.outerHeight(true):l.height.replace("px","");
if(r>p){p=r;}}}f.css("height",p);h.attr("data-current",l.first);h.attr("data-krakatoa",true);if(l.autoplay){l.playing=true;c.fn.krakatoa.play(l,h);h.on("mouseleave",function(){c.fn.krakatoa.play(l,h);
...
if(w===0){x.resolve();}});if(u.height==="auto"){m=v.outerHeight(true);if(m>h){h=m;}}}if(u.height==="auto"){j.css("height",h);}r.attr("data-current",n);
x.done(function(){if(u.playing){c.fn.krakatoa.play(u,r);}l.parent().off("click touchstart","li");l.parent().on("click touchstart","li",{settings:u},a);
});o=Math.round(n/u.items);if(u.buttons){r.find(".active-button").removeClass("active-button").parent().children().eq(o).addClass("active-button");}}function d(f){var h=new Object(),j=[],e=0,g;
if(typeof f!=="string"){return;}f=f.replace(/[{}\s]/g,"");j=f.split(",");for(e;e<j.length;e++){g=j[e].split(":");if(!isNaN(g[1]-0)){if(g[1]%1===0){g[1]=parseInt(g[1]);
}else{g[1]=parseFloat(g[1]);}}else{if(g[1].toLowerCase()=="true"||g[1].toLowerCase()=="false"){g[1]=(g[1]=="true");}}h[g[0]]=g[1];}return h;}}(jQuery));
